...
Each lab gets exactly **one curve**. At each point in time, the curve tracks that lab's **highest-rated** flagship-eligible model, not just the most recently announced one.

- **Highest-Elo wins.** If a lab ships a mid-tier model (e.g. Sonnet) while a higher-tier one (e.g. Opus) still ranks above it, the curve stays on Opus.
